...
export class TalentoHumanoService {
    ...
    private provinciasMap: { [key: string]: string[] } = {
        ...
        'Colombia': ['Antioquia', 'Bogotá D.C.', 'Valle del Cauca', 'Atlántico', 'Bolívar', 'Santander', 'Otros'],
        'Perú': ['Lima', 'Arequipa', 'La Libertad', 'Piura', 'Cusco', 'Callao', 'Otros'],
        'Venezuela': ['Zulia', 'Miranda', 'Carabobo', 'Distrito Capital', 'Aragua', 'Lara', 'Otros'],
        'Argentina': ['Buenos Aires', 'Córdoba', 'Santa Fe', 'Ciudad Autónoma de Buenos Aires', 'Mendoza', 'Tucumán', 'Otros'],
        'Chile': ['Región Metropolitana de Santiago', 'Valparaíso', 'Biobío', 'Antofagasta', 'Araucanía', 'Otros'],
        'Otros': ['Otros']
    };
    ...
    getProvinciasPorPais(pais: string): Observable<string[]> {
        let provincias = this.provinciasMap[pais] || [];
        if (provincias.length === 0 && pais) {
            provincias = ['Otros'];
        }
     ...
     * @param id Identificador de la solicitud.
     * @param estado Nuevo estado (PENDIENTE, EN_REVISION, APROBADO, RECHAZADO, CONTRATADO).
     * @returns Observable con la respuesta del servidor.
     */
    updateEstado(id: number, estado: string): Observable<any> {
        return this.http.put(`${this.apiUrl}/talento-humano/solicitudes/${id}/estado`, { estado });
    }
