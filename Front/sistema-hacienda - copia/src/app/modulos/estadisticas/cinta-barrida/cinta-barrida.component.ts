...
private resetModalState(){
  ...
  cargarColoresPorAño() {
    const anio = this.anioIngresado || this.anioSeleccionado;
     //console.log('Año seleccionado para cargar colores:', anio);
  ...
  onColorChange(event: any) {
    ...
    const selectedColor = this.coloresIds.find(color => color.id === colorId);
    if (selectedColor) {
      ...
      this.selectedTextColor = this.getTextColorForBackground(selectedColor.hex); // Ajustamos el color del texto
    }
  }
  ...
  onSelectChange() {
    ...
    // Verificamos si ambos select tienen un valor seleccionado
    if (this.selectedId && this.selectedCodigo) {
      // @ts-ignore
      const selectedHacienda = this.hacienda.find(dm => dm.id === this.selectedId);
      // Asignamos los nombres seleccionados a las variables
      this.selectedHaciendaName = selectedHacienda ? selectedHacienda.name : 'No disponible';
