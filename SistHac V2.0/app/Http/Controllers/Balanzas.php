    ...
    public function guardarmatascaidas(Request $request)
    {
        ...
        try {
            ...
            $existentes = DB::connection('sql_local')
                ...
                ->map(function ($item) {
                    return $item->pe_seccion . '-' . $item->pe_color;
                })
                ->toArray();
...
", [$id]);
    ...
    public function validarSemana(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            $existe = DB::table('v_historial_matascaidas')
                ->where('semana', $request->semana)
                ->where('anio', $request->anio)
                ->where('idhacienda', $request->idHacienda)
                ->where('codempleado', $request->codEmpleado) // 👈 CAMBIO CLAVE
                ->exists();
